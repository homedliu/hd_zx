...
$(function(){
    ...
    var prBgNow=0;//定义进度条；
    var disparityNum=500;//定义用户和明星初始距离米数；
    var disparityNumNow=disparityNum;//定义用户和明星当前距离米数；
    var prBg=(disparityNum/meterNum)*100;//定义用初始进度条宽度；
    var btnNumMete=Math.ceil(meterNum/btnNum);//定义点击一次走的米数；
    var gameScene=$(".game_scene");
    var peopleRun=$(".people_run");
    //游戏开始初始化

    var numqiRight=((meterNum-disparityNum)/meterNum)*100;//初始红旗的位置；
    var numqiSpeedNum=gameTime*1000/300;//初始红旗明星的速度次数；
    var numqiSpeedMeterNum=numqiRight/numqiSpeedNum;//初始红旗明星的米数速度次数；
    var numqiSpeedNumNow=0;//初始红旗明星的当前速度次数；
    var numqiGoNum=(meterNum-disparityNum)/numqiSpeedNum;//红旗前进的实际米数；
    function gameInit(){
        //btnNumNow=0;
        gameTimeNow=gameTime;
        disparityNumNow=disparityNum;
        $("#meterNow").html(disparityNum+"m");
        //$("#prBg").css("width","1%");
        ...
        $("#num_qi").css("right",numqiRight+"%");
        numqitimeNow=setInterval(function(){
            if(numqiSpeedNumNow<numqiSpeedNum){
                numqiSpeedNumNow++;
                disparityNumNow+=numqiGoNum;
                numqiRight-=numqiSpeedMeterNum;
                if(disparityNumNow>meterNum){
                    disparityNumNow=parseInt(disparityNumNow);
                }
            $("#num_qi").css("right",numqiRight+"%");
                $("#meterNow").html(disparityNumNow+"m");

            }else{
                clearInterval(numqitimeNow);
            }
        },300);
    }
    var prBgWidth=$("#prBg").width();//进度条当前宽度；
    function gameBtn(){
        ...
        prBgWidth+=numqiSpeedMeterNum;
        $("#prBg").css("width",prBgWidth+"%");
        disparityNumNow-=numqiGoNum;
        $("#meterNow").html(disparityNumNow+"m");
        if(100-prBgWidth<numqiRight){
