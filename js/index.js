$(function(){
            ...
            var resources = [
                ...
                img_url+'images/cj_text.png',
                img_url+'images/cj_bg.png',
                img_url+'images/cj_img.png',
                img_url+'images/cj_box_bg.png',
                img_url+'images/no_zhong.png',
                img_url+'images/zhong.png',
                img_url+'images/btn_scth.png'
            ];
            var loadCount=0;
            function loaded(){
                loadCount++;
...
$(function(){
    ...
    $(".btn_playgame").bind('touchstart',function(){
       $(".page_gz_box,.pop_box").fadeOut(300);
        $("#ready_box").show().addClass("ready_ani");
        setTimeout(function(){
            gameInit();
        },4500);
    ...
    var btnNum=140;//定义全程下需按钮次数；
    var distanceNum=300;//定义初始进度条和红旗的距离；
    var hgGoSpeed=300;//定义红旗前进速度，越小越快；


    var hqSeat=(distanceNum/meterNum)*100;//红旗初始位置
    var hqSeatNow=hqSeat;//红旗当前位置
    var btnMeterNum=Math.floor(meterNum/btnNum);//点击一次走的距离长度。注意值最好为整数；
    var hqSeatNowItem=(btnMeterNum/meterNum)*100;//红旗自动走的样式距离。注意值最好为整数；
    var distanceNumNow=distanceNum;//当前进度条和红旗的距离；
    var prBgSeat=0;//进度条当前位置；
    var gameTimeNow=gameTime;//当前游戏剩余时间；
    $("#starman").css("left",hqSeat+"%");
    $("#prBg").css("width",hqSeat+"%");
function gameInit(){
    gameplay=true;
    $("#ready_box").fadeOut(300);
    prBgSeat=0;
    //$("#num_qi").css("right",hqSeat+"%");
    $("#starman").css("left",hqSeat+"%");
    $("#meterNow").html(distanceNum+"m");
    $("#prBg").css("width",hqSeat+"%");
    $("#user_tx").css("left",prBgSeat+"%");
    hqSeatNow=hqSeat;
    ...
    hqGo=setInterval(function(){
        if(hqSeatNow<100)
        {
            ...
            $("#starman").css("left",hqSeatNow+"%");
            $("#prBg").css("width",hqSeatNow+"%");
    ...
    $(".btn_cjyhl").bind('touchstart',function(){
        $(".page_cg_box").fadeOut(300);
        $(".page_cj_box").fadeIn(300);
        $("#gj_con").wScratchPad({
            size        : 15,
            bg          : false,
            fg          : "./images/cj_img.png",
            realtime    : true,
            //     scratchDown : null,
    ...
    $(".btn_cj_close").bind('touchstart',function(){
        $(".page_cj_box").fadeOut(300);
        $(".page_cg_box").fadeIn(300);
    });
});
